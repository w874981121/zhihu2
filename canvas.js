...
function canvasDrawImg(canl, bg, obj, name, callback) {
    ...
    img_bg.onload = function () {
        ...
        img_1.onload = function () {
            ctx.drawImage(img_1, 90, 490, Qingwidth, Qingheight)
            let img_2 = new Image();
            img_2.setAttribute("crossOrigin", 'anonymous')
            img_2.src = obj[1];
            img_2.onload = function () {
                ctx.drawImage(img_2, 90, 655, Qingwidth, Qingheight)
                let img_3 = new Image();
...
function canvasDrawImg2(canl, bg, obj, name, callback) {
    const width = 605;
    const height = 605;
    const topY = 100;
    let canvas = document.getElementById(canl);
    canvas.width = width;
    canvas.height = height + topY;
    let ctx = canvas.getContext("2d");
    let bgimgurl = bg;
    let img_bg = new Image()
    img_bg.setAttribute("crossOrigin", 'anonymous')
    img_bg.src = bgimgurl;
    img_bg.onload = function () {
        ctx.drawImage(img_bg, 2, topY, img_bg.width, img_bg.height)
        // 设置字体
        ctx.font = "40px bold 黑体";
        // 设置颜色
        ctx.fillStyle = "#ffffff";
        ...
        img_1.src = obj[0];
        img_1.onload = function () {
            ctx.drawImage(img_1, 2, 102 + topY, Qingwidth, Qingheight)
            let img_2 = new Image();
            img_2.setAttribute("crossOrigin", 'anonymous')
            img_2.src = obj[1];
            img_2.onload = function () {
                ctx.drawImage(img_2, 2, 276 + topY, Qingwidth, Qingheight)
                let img_3 = new Image();
                img_3.setAttribute("crossOrigin", 'anonymous')
                img_3.src = obj[2];
                img_3.onload = function () {
                    ctx.drawImage(img_3, 2, 450 + topY, Qingwidth, Qingheight)
                    drawImg()
                }
            }
        }
    ...
    function drawImg() {
        let base64Img;
        base64Img = canvas.toDataURL('image/png', 1);
        canvas.width = 0;
        canvas.height = 0;
        callback(base64Img)
    }
